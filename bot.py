...
class InstagramBot:
	def __init__(self, username, password, target):
		...
		self.target = target
		self.bot = webdriver.Chrome("C://Users/Chris/Desktop/chromedriver.exe")	
	...
	def findMyFollowers(self, number_of_followers):
		...
		bot.get('https://instagram.com/' + self.target)
		time.sleep(2)

		bot.find_element_by_xpath('//a[@href="/' + self.target + '/followers/"]').click()
...
insta = InstagramBot('insertEmail', 'insertPassword', 'insertTarget')
insta.login()
